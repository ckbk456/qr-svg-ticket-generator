...
class Pairing:
    def pair(self):
        ...
        i = messagebox.askokcancel(title='Mappings saved',
                                   message=f"Confirm pairings:\n {self.pairings_message}")
        if i:
            self.root.destroy()
        else:
            self.pairings = {}
    ...
    def __init__(self, headers_list):
        ...
        Label(master=self.root,
              text="Keyword and column mapping",
              font=('courier', 12, "bold")).grid(column=0,
                                                 columnspan=len(headers_list),
                                                 row=0,
                                                 padx=10)
        ...
        Button(master=self.root,
               text="save",
               command=self.pair).grid(column=0,
                                       row=3,
                                       columnspan=len(headers_list),
                                       padx=10)
